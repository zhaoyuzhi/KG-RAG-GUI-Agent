def get_action_prompt(instruction, perception_infos, width, height, keyboard, summary_history, action_history, last_summary, last_action, add_info, error_flag, completed_content, memory):
    prompt = "### Background ###\n"
    ...
    prompt += "This information consists of three parts: index; coordinates; content."
    prompt += "The format of the index is the unique number. The format of the coordinates is [x, y], x is the pixel from left to right and y is the pixel from top to bottom; the content is a text or an icon description respectively. "
    prompt += "The information is as follow:\n"
    prompt += get_perceptions_prompts(perception_infos)
    ...
    if add_info != "":
        ...
        prompt += add_info
        prompt += "\nIf the current screenshot does not show any direct link or button to complete the task, choose the optimal action which can move forward the task\n"
        prompt += "\n"
    ...
    prompt += "Tap (index): Tap the widget with the index in current page. You should choose the position of the widget that is clickable.\n"
    prompt += "Swipe (index, direction): Swipe from the block with the index to a specific direction. The direction is choose from (down, up, right, left), You should choose the position of the block that is slideable.\n"
    if keyboard:
        prompt += "Type (index, string): Input the string in the block with the index. The string contain the generated input information.\n"
    else:
    ...
    prompt += "### Action ###\nYou can only choose one from the above actions. Make sure that the coordinates or text in the \"()\".\n"
...
def get_perceptions_prompts(perception_infos):
    prompt = ""
    temp_prompt = ""
    for clickable_info in perception_infos:
        if clickable_info['action_type'] == "CLICK" and clickable_info['desc'] != "" and clickable_info[
            'coordinates'] != (0, 0):
            temp_prompt += f"{clickable_info['index']}; {clickable_info['coordinates']}; Widget description: {clickable_info['desc']}\n"
    if len(temp_prompt):
        prompt += "Those clickable widgets are as follow:\n"
        prompt += temp_prompt

    temp_prompt = ""
    for slideable_info in perception_infos:
        if slideable_info['action_type'] == "SWIPE" and slideable_info['desc'] != "" and slideable_info['coordinates'] != (
                0, 0):
            temp_prompt += f"{slideable_info['index']}; {slideable_info['coordinates']}; Block description: {slideable_info['desc']}\n"
    if len(temp_prompt):
        prompt += "Those slideable blocks are as follow:\n"
        prompt += temp_prompt

    temp_prompt = ""
    for editable_info in perception_infos:
        if editable_info['action_type'] == "EDIT" and editable_info['desc'] != "" and editable_info['coordinates'] != (
                0, 0):
            temp_prompt += f"{editable_info['index']}; {editable_info['coordinates']}; Block description: {editable_info['desc']}\n"
    if len(temp_prompt):
        prompt += "Those editable blocks are as follow:\n"
        prompt += temp_prompt

    return prompt

def get_reflect_prompt(instruction, perception_infos1, perception_infos2, width, height, keyboard1, keyboard2, summary, action, thought):
    ...
    prompt += "Screenshot information:\n"
    prompt += get_perceptions_prompts(perception_infos1)

    prompt += "### After the current operation ###\n"
    prompt += "Screenshot information:\n"
    prompt += get_perceptions_prompts(perception_infos2)


    prompt += "### Current operation ###\n"
    prompt += f"The user\'s ultimate instruction is: {instruction}. In the process of completing the requirements of instruction, an operation is performed on the phone. Below are the details of this operation:\n"
    prompt += "Operation thought: " + thought + "\n"
    prompt += "Operation action: " + action  + "\n"
    prompt += "Operation summary: " + summary.split(" to ")[0].strip() + "\n"
    prompt += "\n"
    ...
    prompt += "### Observation ###\nPlease briefly state the main differences of the before and after screenshots. No more than three sentences\n"
    prompt += "### Thought ###\nBased on the differences of the two screenshot, Your thought about the question\n"
    prompt += "### Answer ###\nA or B or C. Only output the character."
