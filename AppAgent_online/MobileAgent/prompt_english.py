def get_action_prompt(instruction, clickable_infos, width, height, keyboard, summary_history, action_history,
    ...
    prompt = "### Background ###\n"
    prompt += f"This image is a phone screenshot. To help you more accurately describe the action to be performed, we have added grid coordinates to the screenshot.\n The user\'s instruction is: {instruction}.\n\n"
    ...
    if error_flag:
        ...
        last_summary_wo_action = last_summary.split("###")[0]
        prompt += f"You previously wanted to perform the operation \"{last_summary_wo_action}\" on this page and executed the Action \"{last_action}\". But you find that this operation does not meet your expectation. You need to reflect and revise your operation this time."
        prompt += "\n\n"
    ...
    # # prompt += "### Operation ###\nBased on your thought, specify the relevant item in ### Screenshot information ###, and explain the reason for its relevance.\n"
    # prompt += "### Operation ###\nPlease generate a brief natural language description for the operation in Action based on your Thought.\n"
    # prompt += "### Action ###\nYou can only choose one from the six actions above. Make sure that the coordinates or text in the \"()\".\n"
    prompt += "### Observation ###\nDescribe the content of this phone screenshot, especially the parts related to the user\'s instructions.\n"
    prompt += "### Reasoning ###\nBased on the user\' instruction, think about how to complete the task most effectively. Explain what the correct next-step action should be.\n"
    prompt += "### Action ###\nBased on the user\' instruction and your reasoning, correctly choose one from the six actions above. Make sure that the coordinates or text in the \"()\".\n"
