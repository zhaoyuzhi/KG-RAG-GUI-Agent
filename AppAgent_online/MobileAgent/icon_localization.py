...
def det(input_image_path, caption, groundingdino_model, box_threshold=0.05, text_threshold=0.5, processor=None):
    ...
    # inputs = processor(images=image, text=caption, return_tensors="pt").to(groundingdino_model.device)
    # with torch.no_grad():
    ...
    # )
    # boxes_filt = result[0]['boxes']
    # # boxes_filt = torch.tensor(
    # #     [[r['box']['xmin'], r['box']['ymin'], r['box']['xmax'], r['box']['ymax']] for r in result],
    # #     device=groundingdino_model.device
    # # )
    ...
    filtered_boxes = remove_boxes(boxes_filt, size, iou_threshold=0.5)
    coordinates = []
